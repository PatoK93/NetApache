...
namespace Banking.Account.Command.Api.Controllers
{
    ...
    public class BankAccountOperationsAccountController : ControllerBase
    {
        ...
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesDefaultResponseType]
        public async Task<ActionResult<bool>> DepositFund(string id, [FromBody]WithdrawnFundsCommand command)
        {
            command.Id = id;
            return await _mediator.Send(command);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesDefaultResponseType]
        public async Task<ActionResult<bool>> WithdrawnFund(string id, [FromBody] WithdrawnFundsCommand command)
        {
            command.Id = id;
            return await _mediator.Send(command);
        }
